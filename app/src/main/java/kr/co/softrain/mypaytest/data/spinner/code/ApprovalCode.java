package kr.co.softrain.mypaytest.data.spinner.code;
...
public enum ApprovalCode implements RequestCode {
    ...
    public static ApprovalCode getApproval(String code) {
        for (ApprovalCode approvalCode : ApprovalCode.values()) {
            if (approvalCode.code.equals(code)) {
                return approvalCode;
            }
        }
        return null;
    ...
    public static String[] getApprovalArr() {
        ...
        for (int i = 0; i < length; i++) {
            strings[i] = ApprovalCode.values()[i].code;
        }
